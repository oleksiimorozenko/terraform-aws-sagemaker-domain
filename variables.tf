...
variable "default_space_settings" {
  ...
  type = object({
    execution_role  = string                              #  The execution role ARN for the space
    security_groups = optional(list(string))              # A list of security group IDs that will be attached to the space
    ...
    kernel_gateway_app_settings = optional(object({ # The kernel gateway app settings
      ...
      default_resource_spec = optional(object({
        ...
        lifecycle_config_arn          = optional(string) # The Amazon Resource Name (ARN) of the Lifecycle Configuration attached to the Resource
        sagemaker_image_arn           = optional(string) # The ARN of the SageMaker image that the image version belongs to
        sagemaker_image_version_alias = optional(string) # The alias of the image version created on the instance
        sagemaker_image_version_arn   = optional(string) # The ARN of the image version created on the instance
      }))
      lifecycle_config_arns = optional(list(string)) # The Amazon Resource Name (ARN) of the Lifecycle Configurations
    }))
...
variable "default_user_settings" {
  ...
  type = object({
    ...
    canvas_app_settings = optional(object({
      direct_deploy_settings = optional(object({ # The model deployment settings for the SageMaker Canvas application
        enabled = optional(bool, false)          # Describes whether model deployment permissions are enabled or disabled in the Canvas application
      }))
      generative_ai_settings = optional(object({   # The generative AI settings for the SageMaker Canvas application
        amazon_bedrock_role_arn = optional(string) # The ARN of an Amazon Web Services IAM role that allows fine-tuning of large language models (LLMs) in Amazon Bedrock
      }))
      ...
      workspace_settings = optional(object({ # The workspace settings for the SageMaker Canvas application
        s3_artifact_path = optional(string)  # The Amazon S3 bucket used to store artifacts generated by Canvas
        s3_kms_key_id    = optional(string)  # The Amazon Web Services Key Management Service (KMS) encryption key ID that is used to encrypt artifacts generated by Canvas in the Amazon S3 bucket
      }))
    }))
    jupyter_server_app_settings = optional(object({       # The Jupyter server's app settings
      code_repositories_urls = optional(list(string), []) # A list of Git repositories that SageMaker automatically displays to users for cloning in the JupyterServer application
